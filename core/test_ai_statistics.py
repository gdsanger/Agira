...
class AIJobStatisticsViewTests(TestCase):
    ...
    def test_chart_json_not_html_escaped_in_rendered_page(self):
        ...
        # The inline JS assignments for the chart data must not contain HTML-escaped quotes.
        # This targets only the relevant script snippets instead of the whole HTML page.
        req_match = re.search(r"const\\s+reqData\\s*=\\s*(.+?);", content, flags=re.DOTALL)
        dur_match = re.search(r"const\\s+durData\\s*=\\s*(.+?);", content, flags=re.DOTALL)
        self.assertIsNotNone(req_match, "Could not find inline 'const reqData' assignment in page content")
        self.assertIsNotNone(dur_match, "Could not find inline 'const durData' assignment in page content")
        req_snippet = req_match.group(1)
        dur_snippet = dur_match.group(1)
        # If HTML-escaped, double-quotes become &quot; causing JS SyntaxError in these snippets.
        self.assertNotIn('&quot;', req_snippet)
        self.assertNotIn('&quot;', dur_snippet)
